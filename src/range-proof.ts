...
export function createRangeProof(value: number, min: number, max: number, bindingContext?: string): RangeProof {
  if (!Number.isSafeInteger(value) || !Number.isSafeInteger(min) || !Number.isSafeInteger(max)) {
    throw new ValidationError('Range proof values must be safe integers');
  }
  ...
  if (bits > 32) throw new CryptoError('Range too large for range proof (max 2^32)');
...
/**
 * Create an age range proof.
 *
 * @param age - The actual age (secret)
 * @param ageRange - The range to prove, e.g. "8-12" or "18+"
...
/**
 * Serialize a range proof to a JSON string.
 */
...
export function deserializeRangeProof(json: string): RangeProof {
  ...
  try {
    ...
    throw new ValidationError('Invalid range proof: malformed JSON');
  }
  if (typeof parsed !== 'object' || parsed === null) throw new ValidationError('Invalid range proof: not an object');
  const p = parsed as Record<string, unknown>;
  if (typeof p.min !== 'number' || typeof p.max !== 'number') {
    throw new ValidationError('Invalid range proof: missing min/max');
  }
  if (!Number.isSafeInteger(p.min) || !Number.isSafeInteger(p.max) || (p.min as number) < 0 || (p.max as number) < (p.min as number)) {
    throw new ValidationError('Invalid range proof: min/max must be non-negative safe integers with max >= min');
  }
